...
authors:
tags: [s, principal component analysis, statistics, dimensionality reduction, R]
...
<img alt="Lower-triangular correlation matrix for variables Q01 to Q07, with several coefficients circled in orange" style = "float: right; margin-left: 30px;" width = "45%" src="https://pablobernabeu.github.io/2018/naive-principal-component-analysis-in-r/1.jpg" />
...
According to Field et al. (2012), two criteria can be used to determine the number of components that should be carried forward to the next stage:

- SS (sum of squares) loadings, with two possible cut-off points. On the one hand, following Kaiser's threshold, we should select components with SS loadings > 1. In the example result shown above, only PC1 meets this criterion. A more lenient alternative is Kaiser's threshold, whereby SS loadings > .7 are accepted. In the example result above, PC2 meet this criterion too. 

- Scree plot: retain as many components as the number of points after the point of inflection. To create a scree plot, call:

      plot(pc1$values, type = 'b')


<img loading="lazy" decoding="async" alt="Scree plot of PCA eigenvalues against component number, dropping steeply then bending after the first component" style = "float: right; margin-left: 30px;" width = "35%" src="https://pablobernabeu.github.io/2018/naive-principal-component-analysis-in-r/2.jpg" />

Imagine a straight line **from the first point on the right.** Once this line bends considerably, count the points after the bend and up to the last point on the left. The number of points is the number of components to select. The example here is probably the most complicated (two components were finally chosen), but often it's easier [see examples](https://www.google.nl/search?sca_esv=582945116&q=select+principal+components+scree+plot+point+inflection&tbm=isch).

Based on either or both criteria, select the definitive number of components.
...
<img loading="lazy" decoding="async" alt="Loadings table for English and Dutch norms showing auditory, haptic and visual modalities on rotated components RC1 and RC2" style = "float: right; margin-left: 30px;" width = "55%" src="https://pablobernabeu.github.io/2018/naive-principal-component-analysis-in-r/3.jpg" />
...
![Four PCA scatter plots of words on rotated components: English and Dutch properties and concepts](https://pablobernabeu.github.io/2018/naive-principal-component-analysis-in-r/4.png)

An example of these analyses is [available in available in this RStudio environment](https://pablobernabeu.shinyapps.io/Dutch-modality-exclusivity-norms), in the `norms.R` script.
