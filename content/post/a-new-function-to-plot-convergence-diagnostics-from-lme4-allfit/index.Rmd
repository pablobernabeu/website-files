...
authors: []
lastmod: '2021-11-14T19:46:08+01:00'
featured: no
draft: no
image:
  caption: ''
...
plot.fixef.allFit = function(allFit_output, 
  ...
  intercept_plot = intercept %>%
    ...
    theme(axis.title = element_blank(), axis.ticks.x = element_blank(),
          axis.text.x = element_blank(), axis.text.y = ggtext::element_markdown(),
  ...
  predictors_plot = ggplot(predictors, aes(fixed_effect, value, colour = Optimizer)) +
    ...
    facet_wrap(~fixed_effect, scale = 'free',
               ...
               # Wrap names of predictors with more than 55 characters into new lines
               labeller = labeller(group = label_wrap_gen(width = 56))) +
    labs(y = y_title) +
    theme_bw() + theme(axis.title.x = element_blank(), axis.text.x = element_blank(),
                       axis.ticks.x = element_blank(),
                       axis.title.y = element_text(size = 14, margin = margin(0, 15, 0, 0, 'pt')),
                       strip.text = element_text(size = 10, margin = margin(t = 4, b = 6)),
  ...
  if(shared_y_axis_limits == TRUE & !is.null(decimal_points)) {
    ...
    intercept_plot = intercept_plot +
      scale_y_continuous(limits = c(min(allFit_fixef$value) - allFit_fixef$value %>% abs %>% 
                                    ...
                                    max(allFit_fixef$value) + allFit_fixef$value %>% abs %>% 
                                      max / 7 * multiply_y_axis_limits), 
                         # Set number of decimal points and replace minus signs with dashes (\u2013)
                         labels = function(x) {
                           scales::label_number(accuracy = decimal_points)
                           sub('-', '\u2013', x)
                         }
      )
    ...
    intercept_plot = intercept_plot +
      scale_y_continuous(limits = c(min(allFit_fixef$value) - allFit_fixef$value %>% abs %>% 
                                    ...
                                    max(allFit_fixef$value) + allFit_fixef$value %>% abs %>% 
                                      max / 7 * multiply_y_axis_limits),
                         # Replace minus signs with dashes (\u2013)
                         labels = function(x) {
                           sub('-', '\u2013', x)
                         }
      )
