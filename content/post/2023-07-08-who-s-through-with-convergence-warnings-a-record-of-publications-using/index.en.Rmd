...
```{r, fig.width = 7, fig.height = 3.4, fig.alt = 'Bar chart of the number of publications per year whose full text mentions allFit, according to Europe PMC and OpenAlex'}
used_allfit |>
  filter(!is.na(year)) |>
  count(year) |>
  ggplot(aes(year, n)) +
...
The table lists every publication found by the full-text searches, most recent first, with the sources in which it was found. Authors are given as the sources returned them. The two sources overlap in a minority of records, which says as much about their coverage as about the literature.
