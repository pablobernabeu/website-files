...
tags:
  ...
  - bibliometrics
subtitle: ''
summary: 'A convergence warning from lme4 is a diagnostic, not a verdict. One recommended check is to refit with allFit and compare practically relevant estimates across optimisers. This post records candidate publications found through Scopus, Europe PMC and OpenAlex in a reproducible search, and explains why the result is a screening set rather than a complete precedent list.'
abstract: 'This reproducible bibliographic audit searches Scopus, Europe PMC and OpenAlex for publications whose indexed records or full text mention lme4::allFit. It distinguishes direct full-text matches from a broad Scopus proxy, deduplicates records by DOI and presents the result as a lower-bound screening set rather than evidence that every listed study used allFit to resolve a convergence warning.'
authors: []
lastmod: ''
featured: no
draft: no
...
Mixed-effects models with the random-effects structure that the design justifies (Barr et al., 2013) can end their fit with an lme4 warning that an optimiser may not have converged. The warning reports that a numerical check exceeded a tolerance; it does not, by itself, establish that the fitted model is wrong. The lme4 documentation notes that approximate derivative calculations can yield false positives, particularly for large or complex fits, but warnings still warrant investigation. A model with too little information to estimate every variance and correlation may also show boundary estimates, such as a variance near zero or a correlation near one (Bates et al., 2015; Matuschek et al., 2017).

One diagnostic recommended by lme4 is to refit the model with the available optimisers through `allFit()` and compare estimates that matter for the intended conclusions. Practical agreement is consistent with a false-positive warning, but it does not prove that the model is otherwise appropriate; disagreement calls for closer inspection of the data, specification and optimisation settings. I wrote about the practice and published a [plotting function for the comparison](/2023/a-new-function-to-plot-convergence-diagnostics-from-lme4-allfit/) some years ago. This post keeps a reproducible record of candidate publications that can be found by searching, so that readers can inspect the evidence rather than treat the list as a definitive catalogue.
...
The two full-text queries are shown as the workflow recorded them, so that what
appears here is what actually ran.
