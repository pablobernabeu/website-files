...
# Natural Earth's medium geometry carries about 100,000 vertices, which is 3 MB
# of coordinates in the widget below. A 5 km simplification tolerance is
# invisible at world zoom and removes about three fifths of that. Two classes of
# feature are exempted, because simplifying them damages the map: those crossing
# the antimeridian, whose longitudes wrap and which are redrawn as bands across
# the plate if their vertices are moved, and the smallest polygons, which
# simplification empties altogether.
simplify_for_web <- function(x, tolerance_m = 5000) {
  longitude_span <- vapply(st_geometry(x), function(g) {
    box <- st_bbox(g); unname(box[['xmax']] - box[['xmin']])
  }, numeric(1))
  y <- x
  local <- longitude_span <= 350
  y[local, ] <- suppressWarnings(
    st_simplify(x[local, ], dTolerance = tolerance_m, preserveTopology = TRUE))
  emptied <- st_is_empty(y)
  y[emptied, ] <- x[emptied, ]
  y
}
