...
abstract: 'This article analyses 2024 greenhouse-gas emissions using the 2025 EDGAR release, separating total emissions from per-capita emissions and reading both against income group and human development. It documents the data preparation, visualises regional totals, leading emitters and sectors, and uses transparent residuals to show where countries emit more or less than an HDI-based expectation.'
authors: []
lastmod: ''
featured: no
draft: no
...
The analysis is organised as a visual argument: define the data and denominators, compare totals with per-capita values, inspect sector composition and finally model the relationship between development and emissions. Each plot states its unit, population definition and source so that the comparisons are not mistaken for a single measure of responsibility.
...
The table shows how different the two ends are in kind. The positive end is dominated by fuel exporters and by pastoral or very small economies. The negative end has no single cause, and ranges from an HDI of `r fmt(min(hdi_data$hdi_2023[order(hdi_data$residual)][1:10]), 2)` to `r fmt(max(hdi_data$hdi_2023[order(hdi_data$residual)][1:10]), 2)`.


## Discussion

The figures support a contextual reading of emissions, not a single ranking of countries. Total emissions describe the scale of a country's contribution to the global atmospheric problem; per-capita values describe an average exposure or responsibility per resident; sector shares suggest where mitigation could begin; and the HDI residuals show departures from a broad development relationship. None of these quantities is a causal estimate of climate responsibility. They are complementary summaries whose interpretation depends on the boundary choices documented above, especially the exclusion of land-use change and the treatment of international transport.
