...
The next chunk reads the sheets of totals and per-capita emissions, moves the years from columns into rows, derives the population of each territory and year as the ratio of the total to the per-capita figure (Mt divided by tonnes per person gives millions of people) and checks that the country rows plus international aviation and shipping add up to EDGAR's global total.
...
Figure 3 ranks countries by their 2024 total, coloured by continent, with each country's share of the world total printed at the end of its bar and its per-capita emissions in a dot plot beside it. The original version drew confidence intervals around these bars, computed as the standard error of a mean across years, which measure the year-to-year variation of a series and say nothing about the uncertainty of a sum, so they are omitted here.
...
world_per_capita <- world_2024 / world_population$world_population[world_population$year == 2024] * 1e6
...
p3a + p3b +
  ...
  plot_annotation(
    caption = paste0(edgar_caption, ' Units: million tonnes of CO<sub>2</sub> equivalent (panel a)',
                     ...
                     'The dotted line in panel b is the world average of ',
...
largest_sector <- sector_shares %>%
  ...
  slice_max(share, n = 1) %>%
  ungroup()
countries_led_by <- function(sec) paste(largest_sector$country[largest_sector$sector == sec], collapse = ' and ')
...
The largest positive residual belongs to `r hdi_data$country[which.max(hdi_data$residual)]`, at `r fmt(max(hdi_data$ratio), 1)` times its expected level, and the largest negative one to `r hdi_data$country[which.min(hdi_data$residual)]`, at `r fmt(min(hdi_data$ratio), 2)` times. The table below lists the ten countries at each end. Oil and gas exporters (Qatar, Kuwait, Bahrain, Turkmenistan, Brunei and Libya) and small states whose per-capita figure is set by one power plant or one fleet (Palau, with `r fmt(hdi_data$population[hdi_data$iso3 == 'PLW'] / 1e3)` thousand inhabitants, and Trinidad and Tobago) fill the positive end, together with Mongolia, where fuel extraction and livestock account for `r pct(sum(sectors_2024$mt[sectors_2024$iso3 == 'MNG' & sectors_2024$sector %in% c('Fuel Exploitation', 'Agriculture')]) / sum(sectors_2024$mt[sectors_2024$iso3 == 'MNG']) * 100, 0)` of emissions, and with Chad and the Central African Republic, where agriculture makes up `r pct(sectors_2024$mt[sectors_2024$iso3 == 'TCD' & sectors_2024$sector == 'Agriculture'] / sum(sectors_2024$mt[sectors_2024$iso3 == 'TCD']) * 100, 0)` and `r pct(sectors_2024$mt[sectors_2024$iso3 == 'CAF' & sectors_2024$sector == 'Agriculture'] / sum(sectors_2024$mt[sectors_2024$iso3 == 'CAF']) * 100, 0)` of emissions and methane from herds counts in full at a low HDI. The negative end mixes small island states, countries of South Asia and Sub-Saharan Africa with low emissions for their HDI and, at the top of the HDI range, Switzerland (reported by EDGAR together with Liechtenstein), whose electricity comes largely from hydropower and nuclear plants. Because EDGAR reports Sudan and South Sudan as one territory, their combined per-capita figure is compared with the HDI of Sudan and mapped onto both countries.
