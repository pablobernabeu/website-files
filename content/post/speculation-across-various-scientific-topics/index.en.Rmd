...
Across the whole period, the literature on `r totals$topic[1]` has the highest share of records mentioning speculation, at `r sprintf('%.1f', totals$share[1])`%. The others run from `r sprintf('%.1f', totals$share[2])`% down to `r sprintf('%.1f', totals$share[nrow(totals)])`%, and neighbouring figures are not separated by the counts they rest on. The lowest belongs to `r totals$topic[nrow(totals)]`, whose `r totals$records[nrow(totals)]` records are too few to place it anywhere in that order.
