...
The function counts, for every year, the records matching the reference query and the records matching both the reference query and the comparison term, so the nine comparisons cost `r 9 * 2 * length(1980:2026)` count requests. Each row of the result holds a year, the count for the reference literature, the count for the comparison and the comparison as a percentage of the reference.
...
Across the whole period, the share of records that mention speculation is highest in `r totals$topic[1]` (`r sprintf('%.1f', totals$share[1])`%) and lowest in `r totals$topic[nrow(totals)]` (`r sprintf('%.1f', totals$share[nrow(totals)])`%), with the language literatures spread between those two.
...
Each panel below shows one literature. The line is the yearly percentage of records that also match `speculat*`, the shaded band is a Wilson interval computed from the yearly reference count, which conveys how much the share would move with a handful of records, and the panels are ordered from the most to the least speculative literature over the whole period. The axis is cut at 15%, which clips the bands in the early years of the small literatures, where a single abstract would move the share by tens of points. Years in which a literature had no records are left blank.
...
Restricting the comparison to the last ten complete years, `r min(recent$records)` to `r format(max(recent$records), big.mark = ',')` records per literature, gives the ranking above. `r recent$topic[1]` leads with `r sprintf('%.1f', recent$share[1])`% of its records mentioning speculation, and `r recent$topic[nrow(recent)]` closes the list with `r sprintf('%.1f', recent$share[nrow(recent)])`%.
...
With those limits in mind, the pattern across the language sciences is consistent with the impression that started the exercise. Literatures that theorise about unobservable or contested processes carry more explicit speculation than literatures organised around a clinical population, a classroom or an instrument, and the difference is visible in abstracts before any reading of the articles themselves. Whether the speculation is a symptom of immaturity or a healthy habit of labelling conjecture is a question that counts cannot answer. The searches can be rerun as the literatures grow, and the workflow that produces them is in the repository for anyone who wants to add a topic.
