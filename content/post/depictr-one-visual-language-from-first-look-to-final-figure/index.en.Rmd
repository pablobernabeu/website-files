...
```{r distribution, fig.alt = 'Empirical cumulative distributions of lexical-decision reaction times for related and unrelated primes, with horizontal guides at the quartiles'}
ecdf_plot(
  lexical_decision,
  RT,
  group = condition,
...
```{r model-estimates, fig.height = 3.3, fig.alt = 'Forest plot of the fixed effects, each per standard deviation of its predictor, for unrelated priming, auditory presentation and word frequency, with 95 per cent confidence intervals'}
coefficient_plot(
  fit,
  standardise = TRUE,
  order = 'ascending',
...
The forest plot keeps the colours and typography of the distribution plot, so a reader moving from observations to estimates can concentrate on the result.
...
The [R reference](https://pablobernabeu.github.io/depictr/reference/) and [Python reference](https://pablobernabeu.github.io/depictr-py/api/) cover the rest of the package, including diagnostics, posterior plots, power curves, model comparisons and multi-panel composition.
