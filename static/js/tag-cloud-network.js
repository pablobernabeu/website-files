  ...
  function setupTagHoverEffects(tags, cooccurrences) {
    ...
    tags.forEach(tag => {
      ...
      tag.element.addEventListener('mouseenter', () => {
        // Highlight tags (using pre-cached data) - simplified for instant response
        const tagData = tagConnectionsMap.get(tag.name);
        
        // Apply styles directly without calculations first
        tag.element.style.cssText = `
          color: ${tag.cachedColor} !important;
          outline: 2px solid ${tag.cachedColor} !important;
          outline-offset: 2px !important;
          border-radius: 0.3em !important;
          background-color: ${tag.cachedColor}30 !important;
          text-shadow: 0 0 12px ${tag.cachedColor}60 !important;
          opacity: 1 !important;
        `;
